"use client"

import type React from "react"

import { useRef, useState } from "react"
import { Button } from "@/components/ui/button"
import { Card } from "@/components/ui/card"
import { FileText, ImageIcon, Music, X, Upload, Play, CheckCircle, AlertCircle, Loader2 } from "lucide-react"
import { cn } from "@/lib/utils"
import { fileUploadService, type UploadedFile } from "@/lib/file-upload-service"
import { authService } from "@/lib/auth-service"

interface FileUploadSidebarProps {
  files: File[]
  onFilesAdded: (files: File[]) => void
  onFileRemove: (index: number) => Promise<void>
  onServerFileRemove?: (fileId: string) => Promise<void>
  onStartProcessing: () => void
  isProcessing: boolean
  uploadedFiles?: UploadedFile[]
  onUploadedFilesChange?: (files: UploadedFile[]) => void
}

export function FileUploadSidebar({
  files,
  onFilesAdded,
  onFileRemove,
  onStartProcessing,
  isProcessing,
  uploadedFiles = [],
  onUploadedFilesChange,
  onServerFileRemove,
}: FileUploadSidebarProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const dropZoneRef = useRef<HTMLDivElement>(null)
  const [uploadingFiles, setUploadingFiles] = useState<Set<string>>(new Set())
  const [uploadErrors, setUploadErrors] = useState<Map<string, string>>(new Map())

  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFiles = Array.from(e.target.files || [])
    await handleFilesUpload(selectedFiles)
  }

  const handleDrop = async (e: React.DragEvent) => {
    e.preventDefault()
    const droppedFiles = Array.from(e.dataTransfer.files)
    await handleFilesUpload(droppedFiles)
  }

  const handleFilesUpload = async (filesToUpload: File[]) => {
    // Validate files first
    const validFiles: File[] = []
    const invalidFiles: { file: File; reason: string }[] = []

    for (const file of filesToUpload) {
      if (!fileUploadService.isValidFileType(file)) {
        invalidFiles.push({ file, reason: 'Unsupported file type' })
      } else if (!fileUploadService.isValidFileSize(file)) {
        invalidFiles.push({ file, reason: 'File too large (max 100MB)' })
      } else {
        validFiles.push(file)
      }
    }

    // Show invalid files
    if (invalidFiles.length > 0) {
      alert(`Some files were rejected:\n${invalidFiles.map(f => `${f.file.name}: ${f.reason}`).join('\n')}`)
    }

    if (validFiles.length === 0) return

    // Add valid files to local state
    onFilesAdded(validFiles)

    // Upload files to GCS
    for (const file of validFiles) {
      await uploadFileToGCS(file)
    }
  }

  const uploadFileToGCS = async (file: File) => {
    const fileKey = `${file.name}-${file.size}`
    
    try {
      setUploadingFiles(prev => new Set(prev).add(fileKey))
      setUploadErrors(prev => {
        const newMap = new Map(prev)
        newMap.delete(fileKey)
        return newMap
      })

      // Set auth token if available
      const token = authService.getAuthToken()
      if (token) {
        fileUploadService.setAuthToken(token)
      }

      const uploadedFile = await fileUploadService.uploadFile(file)
      
      // Add to uploaded files
      if (onUploadedFilesChange) {
        onUploadedFilesChange([...uploadedFiles, uploadedFile])
      }

    } catch (error) {
      console.error('Upload error:', error)
      setUploadErrors(prev => new Map(prev).set(fileKey, error instanceof Error ? error.message : 'Upload failed'))
    } finally {
      setUploadingFiles(prev => {
        const newSet = new Set(prev)
        newSet.delete(fileKey)
        return newSet
      })
    }
  }

  const handleDragOver = (e: React.DragEvent) => {
    e.preventDefault()
  }

  const getFileIcon = (file: File) => {
    if (file.type.startsWith("image/")) return <ImageIcon className="h-4 w-4" />
    if (file.type.startsWith("audio/")) return <Music className="h-4 w-4" />
    return <FileText className="h-4 w-4" />
  }

  const formatFileSize = (bytes: number) => {
    return fileUploadService.formatFileSize(bytes)
  }

  const getFileStatusIcon = (file: File) => {
    const fileKey = `${file.name}-${file.size}`
    
    if (uploadingFiles.has(fileKey)) {
      return <Loader2 className="h-4 w-4 animate-spin text-blue-500" />
    }
    
    if (uploadErrors.has(fileKey)) {
      return <AlertCircle className="h-4 w-4 text-red-500" />
    }
    
    const uploadedFile = uploadedFiles.find(uf => uf.file_name === file.name && uf.file_size === file.size)
    if (uploadedFile) {
      return <CheckCircle className="h-4 w-4 text-green-500" />
    }
    
    return null
  }

  const getFileStatusText = (file: File) => {
    const fileKey = `${file.name}-${file.size}`
    
    if (uploadingFiles.has(fileKey)) {
      return "Uploading..."
    }
    
    if (uploadErrors.has(fileKey)) {
      return uploadErrors.get(fileKey) || "Upload failed"
    }
    
    const uploadedFile = uploadedFiles.find(uf => uf.file_name === file.name && uf.file_size === file.size)
    if (uploadedFile) {
      return uploadedFile.status === 'processing' ? 'Processing...' : 'Uploaded'
    }
    
    return "Ready to upload"
  }

  return (
    <div className="flex w-80 flex-col border-r border-border bg-sidebar">
      <div className="border-b border-sidebar-border p-4">
        <h2 className="text-lg font-semibold text-sidebar-foreground">Study Materials</h2>
        <p className="text-sm text-muted-foreground">Upload your learning resources</p>
      </div>

      <div className="flex-1 overflow-y-auto p-4">
        <div
          ref={dropZoneRef}
          onDrop={handleDrop}
          onDragOver={handleDragOver}
          className={cn(
            "mb-4 rounded-lg border-2 border-dashed border-border bg-muted/30 p-6 text-center transition-colors hover:border-primary/50 hover:bg-muted/50",
            isProcessing && "pointer-events-none opacity-50",
          )}
        >
          <Upload className="mx-auto mb-2 h-8 w-8 text-muted-foreground" />
          <p className="mb-2 text-sm font-medium text-foreground">Drop files here</p>
          <p className="mb-3 text-xs text-muted-foreground">Audio, images, text, or PowerPoint files</p>
          <Button variant="secondary" size="sm" onClick={() => fileInputRef.current?.click()} disabled={isProcessing}>
            Browse Files
          </Button>
          <input
            ref={fileInputRef}
            type="file"
            multiple
            accept="audio/*,image/*,text/*,.pdf,.ppt,.pptx,.doc,.docx"
            onChange={handleFileSelect}
            className="hidden"
          />
        </div>

        <div className="space-y-2">
          {/* Local pending files (not yet uploaded) */}
          {(() => {
            const uploadedKeys = new Set(uploadedFiles.map(uf => `${uf.file_name}-${uf.file_size}`))
            const localUnique = files.filter(f => !uploadedKeys.has(`${f.name}-${f.size}`))
            return localUnique
          })().map((file, index) => (
            <Card key={index} className="p-3">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 text-muted-foreground">{getFileIcon(file)}</div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-card-foreground">{file.name}</p>
                  <p className="text-xs text-muted-foreground">{formatFileSize(file.size)}</p>
                  <div className="flex items-center gap-2 mt-1">
                    {getFileStatusIcon(file)}
                    <p className="text-xs text-muted-foreground">{getFileStatusText(file)}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6 shrink-0"
                  onClick={async () => {
                    // Handle async deletion properly
                    try {
                      await onFileRemove(index)
                    } catch (error) {
                      console.error('Error removing file:', error)
                    }
                  }}
                  disabled={isProcessing || uploadingFiles.has(`${file.name}-${file.size}`)}
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </Card>
          ))}
          {/* Server-side uploaded files from database */}
          {uploadedFiles.length > 0 && uploadedFiles.map((uf, index) => (
            <Card key={`uploaded-${uf.file_id}-${index}`} className="p-3">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 text-muted-foreground"><FileText className="h-4 w-4" /></div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-card-foreground">{uf.file_name}</p>
                  <p className="text-xs text-muted-foreground">{formatFileSize(uf.file_size)}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <CheckCircle className="h-4 w-4 text-green-500" />
                    <p className="text-xs text-muted-foreground">Uploaded</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6 shrink-0"
                  onClick={async () => {
                    if (!onServerFileRemove) return
                    try {
                      await onServerFileRemove(uf.file_id)
                    } catch (error) {
                      console.error('Error removing server file:', error)
                    }
                  }}
                  disabled={isProcessing}
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </Card>
          ))}
        </div>
      </div>

      <div className="border-t border-sidebar-border p-4">
        <Button 
          className="w-full" 
          onClick={onStartProcessing} 
          disabled={uploadedFiles.length === 0 || isProcessing}
        >
          <Play className="mr-2 h-4 w-4" />
          {isProcessing ? "Processing..." : "Start Learning Session"}
        </Button>
        {uploadedFiles.length > 0 && !isProcessing && (
          <p className="mt-2 text-center text-xs text-muted-foreground">
            {uploadedFiles.length} document{uploadedFiles.length !== 1 ? "s" : ""} ready
          </p>
        )}
        {uploadedFiles.length === 0 && !isProcessing && (
          <p className="mt-2 text-center text-xs text-muted-foreground">
            Upload files to get started
          </p>
        )}
      </div>
    </div>
  )
}
